...
- 按节点类型渲染 10 种不同颜色和形态的三维目标图标，其中包含 2 个按 IP 识别的特殊类型。
...
节点类型枚举以基础 8 类加 2 个按 IP 识别的特殊类型组成，共 10 类：
...
其中类型 `30` 和 `31` 不直接来自后端 `node_type`，而是前端在归一化阶段基于 `node_manage_ip_addr` 识别后覆盖得到。
...
- 缺少 `node_id`：丢弃该节点。
- `node_type` 不在基础集合 `1` 到 `8`，且 IP 也未命中特殊类型名单：使用“未知类型”兜底样式，或不渲染。第一阶段建议使用兜底样式并在标牌中显示原始类型值。
...
- `LegendPanel`：10 种节点类型全部展示，文本与配置一致，其中无人机和手持背负设备类型可正常显示。
