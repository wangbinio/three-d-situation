...
| 00-05 | 实现 `nodeTypeStyle.ts` | 定义基础 1 到 8 类节点类型，加上无人机 `30` 特殊类型的中文名、形态、颜色、模型路径、图例文本和兜底样式。 | 00-03 |
| 00-06 | 为节点类型样式写单测 | 校验 9 类完整、颜色非空、未知类型走兜底样式。 | 00-05 |
...
- `nodeTypeStyle.ts` 覆盖 9 种节点类型，并提供未知类型兜底。
- 单测覆盖配置解析和节点类型样式。
- 类型检查、单测和构建通过。
